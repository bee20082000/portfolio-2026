...
const TILE_WIDTH = "span 8";
const TILE_RADIUS = "5px";
const TILE_HEIGHT = "span 12";

export default function MoeTile({ onSelect, index }) {
  ...
  return (
    <div
      ref={myTileRef}
      className={`tile tile-case ${styles['tile-case']} tile-moe`}
      data-i={index}
      onClick={handleClick}
    ...
    >
      ...
      >
        <div
          ...
          style={{
            position: 'absolute',
            inset: 0,
            backgroundImage: `url('/asset/images/Moe Cafe/moe cover.jpeg')`,
      ...
      <div ref={labelBarRef} className="tile-label-bar">
        <span className="brand-name">Moe Cafe and Gift Shop</span>
        <span className="project-name">Packaging and Branding</span>
      </div>
    </div>
  );
}
